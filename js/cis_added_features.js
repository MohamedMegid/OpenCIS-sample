Drupal.behaviors.cis_added_features = function() {
  ...
  $(document).ready(function() {
    ...
    control_outside_repair_info_tab();
    ...
    search_by_group_or_engineer_in_performance_evluation();
...
/**
 * search by group or by engineer in
 * performance evaluation report
 */
function search_by_group_or_engineer_in_performance_evluation(){
  var path = location.pathname.split("/");

  if ( path[2] == "performance" && path[3] == "evaluation" ) {
    $('#edit-group-eval-wrapper').hide();
  }
  var group = getUrlParameter('group');
  if (group){
    $('#edit-group-eval-wrapper').show();
    $('#edit-engineer-wrapper').hide();
  }
  $('#edit-search-by-group').change(function(event){
    if(this.checked) {
      $('#edit-group-eval-wrapper').show();
      $('#edit-engineer-wrapper').hide();
      $('#edit-engineer').val('None');
    }
    else{
      $('#edit-group-eval-wrapper').hide();
      $('#edit-engineer-wrapper').show();
      $('#edit-group-eval').val('None');
    }
  });
}

/**
 * hide/show outside repair info tab
 */
function control_outside_repair_info_tab(){
  var wo_type = $('#edit-field-wrk-order-type-value').val();
  if (wo_type == 1){
    var first_action_status = $('#edit-field-wrk-order-m-situation-value').val();
    var work_order_type = $('#edit-field-wrk-order-type-value').val();
    if (work_order_type == 1){
      if (first_action_status != 24){
        $('.horizontal-tab-button-2').attr("style", "display : none !important");
      }
    }
    if (work_order_type == 2){
      $('.horizontal-tab-button-2').attr("style", "display : none !important");
    }
    $('#edit-field-wrk-order-m-situation-value').change(function(event) {
      var first_action_status = $('#edit-field-wrk-order-m-situation-value').val();
      var work_order_type = $('#edit-field-wrk-order-type-value').val();
      if (work_order_type == 1){
        if (first_action_status == 24){
